...
export function YouTubePlaylistPlayer({
        ...
        events: {
          ...
          onStateChange: (event: any) => {
            ...
            if (event.data === 0) {
              ...
              if (currentVideo.id && !completedIds.has(currentVideo.id) && onCompleteLesson) {
                setCompletedIds((prev) => {
                  const next = new Set(prev);
                  next.add(currentVideo.id!);
                  return next;
                });
                onCompleteLesson(currentVideo.id);
              }
      ...
      try {
        ...
        if (player && player.getCurrentTime && player.getDuration) {
          ...
          if (d > 0) {
            ...
            // Mark lesson complete automatically at 80% progress
            if (progress >= 80 && currentVideo.id && !completedIds.has(currentVideo.id) && !currentVideo.completed) {
              setCompletedIds((prev) => {
                const next = new Set(prev);
                next.add(currentVideo.id!);
                return next;
              ...
              if (onCompleteLesson) {
                await onCompleteLesson(currentVideo.id);
              }
            }
          }
        }
  ...
  }, [isPlayerReady, currentVideo, completedIds, onCompleteLesson]);
          ...
          <div className="w-72 bg-zinc-900/95 border-s border-white/10 flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            <div className="flex-1 overflow-y-auto py-1">
              {videos.map((video, idx) => {
                ...
                const isCompleted = video.completed || (video.id && completedIds.has(video.id));
                return (
                  <button
                    key={video.videoId}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-[11px] font-medium leading-tight line-clamp-2 ${isActive ? 'text-red-400' : isCompleted ? 'text-white/40 line-through' : 'text-white/90'}`}>
                        {video.title}
                      </p>
                      <p className={`text-[9px] mt-0.5 ${isActive ? 'text-red-400/70' : 'text-white/40'} flex items-center gap-1`}>
                        {isCompleted && <Check className="w-3 h-3 text-green-400 flex-shrink-0" />}
                        <span>#{video.position + 1} • {video.channelTitle}</span>
                      </p>
                    </div>
                    {isActive && (
